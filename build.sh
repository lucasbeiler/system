...
apk --keys-dir=/usr/share/apk/keys/ -p $WORKDIR --arch $ARCH --initdb --no-cache add alpine-base alpine-baselayout acpid busybox busybox-suid musl-utils openssl argon2 fortify-headers iwd e2fsprogs e2fsprogs-extra util-linux tpm2-tools tpm2-tss-dev tpm2-tss-tcti-device fscrypt fscryptctl \
  ...
  alsa-utils alsaconf pipewire-alsa pipewire-jack
...
chroot $WORKDIR rc-update add acpid default
chroot $WORKDIR rc-update add bootmisc boot
chroot $WORKDIR rc-update add crond default
...
chroot $WORKDIR rc-update add savecache shutdown
chroot $WORKDIR rc-update add seedrng boot
chroot $WORKDIR rc-update add setup-crosvm-net default
chroot $WORKDIR rc-update add swap boot
...
rm -rf ${WORKDIR}/home/ ${WORKDIR}/var/lib/iwd/ ${WORKDIR}/var/empty/ ${WORKDIR}/var/lock/ ${WORKDIR}/var/log/
...
chroot ${WORKDIR} ln -sf /data/var/log /var/log
chroot ${WORKDIR} ln -sf /data/var/lock /var/lock
mkdir ${WORKDIR}/data
