...
apk --keys-dir=/usr/share/apk/keys/ -p $WORKDIR --arch $ARCH --initdb --no-cache add alpine-base alpine-baselayout acpid busybox busybox-suid musl-utils openssl argon2 fortify-headers iwd e2fsprogs e2fsprogs-extra util-linux tpm2-tools tpm2-tss-dev tpm2-tss-tcti-device fscrypt fscryptctl \
  ...
  alsa-utils alsaconf pipewire-alsa pipewire-jack font-jetbrains-mono-nerd font-iosevka font-awesome font-liberation font-noto font-noto-emoji power-profiles-daemon mtools device-mapper libcap-getcap libcap-setcap helix bubblewrap-static libseccomp-dev
...
gunzip /usr/local/bin/spotify_player.gz
EOF
