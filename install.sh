...
RELEASE_TAG="$(curl -s https://api.github.com/repos/lucasbeiler/system/releases | grep -i "os-" | grep '"name"' | head -1 | cut -d'"' -f4 | sed 's/[^0-9]//g')"
...
curl -L -f --progress-bar -o "$WORKDIR/bootloader-signed.efi"                "$RELEASE_URL/bootloader-signed.efi"
curl -L -f --progress-bar -o "$WORKDIR/uki-${RELEASE_TAG}-signed.efi"        "$RELEASE_URL/uki-${RELEASE_TAG}-signed.efi"
curl -L -f --progress-bar -o "$WORKDIR/rootfs.erofs"                         "$RELEASE_URL/rootfs.erofs"
curl -L -f --progress-bar -o "$WORKDIR/rootfs.erofs.verity"                  "$RELEASE_URL/rootfs.erofs.verity"
...
# 5. Write rootfs.erofs to root_a (and its rootfs.erofs.verity too verity_a)
info "Writing rootfs.erofs to root_a (${P}2)..."
dd if="$WORKDIR/rootfs.erofs" of="${P}2" bs=4M status=progress
sync
info "Writing rootfs.erofs.verity to verity_a (${P}3)..."
dd if="$WORKDIR/rootfs.erofs.verity" of="${P}3" bs=4M status=progress
sync
