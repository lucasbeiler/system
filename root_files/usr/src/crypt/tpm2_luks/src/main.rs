...
const LUKS_DEVICE: &str = "/dev/disk/by-partlabel/data";
...
fn setup(password: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let device_path = PathBuf::from(LUKS_DEVICE);
...
fn unlock(password: &str) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    let device_path = PathBuf::from(LUKS_DEVICE);
...
fn usage(program: &str) {
    eprintln!("Usage: {} <setup|unlock>", program);
    eprintln!();
