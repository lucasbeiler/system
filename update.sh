...
# 2. Write rootfs and verity to the inactive slots
info "Writing rootfs.squashfs to $TARGET_ROOT..."
dd if="$WORKDIR/rootfs.squashfs" of="$TARGET_ROOT" bs=4M status=progress
sync

info "Writing rootfs.squashfs.verity to $TARGET_VERITY..."
dd if="$WORKDIR/rootfs.squashfs.verity" of="$TARGET_VERITY" bs=4M status=progress
sync

# 3. Update bootloader and UKI in ESP
info "Updating ESP..."
